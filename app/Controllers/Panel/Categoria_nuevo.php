...
class Categoria_nuevo extends BaseController
{
    ...
    {
        $session = session();
        if (!Permisos::is_rol_permitido(TAREA_CATEGORIA_NUEVO, isset($session->rol_actual['clave']) ? $session->rol_actual['clave'] : -1)) {
            $this->permitido = false;
        } //end if rol no permitido
        else {
            $session->tarea_actual = TAREA_CATEGORIA_NUEVO;
    ...
    {
        if ($this->permitido) {
            return $this->crear_vista("panel/categoria_nuevo", $this->cargar_datos());
    ...
    private function cargar_datos()
    {
        ...
        //======================================================================
        $datos['nombre_pagina'] = 'Nuevo Categoría';
        //Cargar modelos
        ...
        $navegacion = array(
            ...
            array(
                'tarea' => 'Nuevo Categoría',
                'href' => '#'
            )
        );
        $datos['breadcrumb'] = breadcrumb_panel($navegacion, 'Nuevo Categoría');
    ...
    public function registrar()
    {
        if ($this->permitido) {
            ...
            try {
                $tabla_Categorias->insert($categoria);

                mensaje("La categoría ha sido registrada exitosamente.", SUCCESS_ALERT, "¡Registro exitoso!");
                return redirect()->to(route_to('administracion_categorias'));
            } catch (\Exception $e) {
                mensaje("Hubo un error al registrar la categoría. Intente nuevamente, por favor", DANGER_ALERT, "¡Error al registrar!");
                return $this->index();
            }
        } else {
            return $this->index();
