...
class Productos_categorias extends BaseController
{
	...
	{
		$session = session();
		if (!Permisos::is_rol_permitido(TAREA_PRODUCTOS_CATEGORIAS, isset($session->rol_actual['clave']) ? $session->rol_actual['clave'] : -1)) {
			$this->permitido = false;
		} //end if rol no permitido
		else {
			$session->tarea_actual = TAREA_PRODUCTOS_CATEGORIAS;
	...
	{
		if ($this->permitido) {
			return $this->crear_vista("panel/asignacion_categorias", $this->cargar_datos());
	...
	private function cargar_datos()
	{
		...
		//======================================================================
		$datos['nombre_pagina'] = 'Asignación Categorías';

		//Cargar modelos
		$tabla_productos_categorias = new \App\Models\Tabla_producto_categoria;
		$datos['productos_categorias'] = $tabla_productos_categorias->datatable_producto_categoria();

		//Breadcrumb
		$navegacion = array(
			array(
				'tarea' => 'Asignación Categorías',
				'href' => '#'
			)
		);
		$datos['breadcrumb'] = breadcrumb_panel($navegacion, 'Asignación Categorías');
