...
class Dashboard extends BaseController
{
	...
	{
		$session = session();
		if (!Permisos::is_rol_permitido(TAREA_TRABAJADOR_DASHBOARD, isset($session->rol_actual['clave']) ? $session->rol_actual['clave'] : -1)) {
			$this->permitido = false;
		} //end if rol permitido
		else {
			$session->tarea_actual = TAREA_TRABAJADOR_DASHBOARD;
		} //end else rol permitido
	} //end constructor
	public function index()
	{
		if ($this->permitido) {
			return $this->crear_vista("panel/dashboard_trabajador", $this->cargar_datos());
		} //end if rol permitido
		else {
