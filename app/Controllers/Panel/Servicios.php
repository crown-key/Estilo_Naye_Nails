...
class Servicios extends BaseController
{
	...
	{
		$session = session();
		if (!Permisos::is_rol_permitido(TAREA_SERVICIOS, isset($session->rol_actual['clave']) ? $session->rol_actual['clave'] : -1)) {
			$this->permitido = false;
		} //end if rol no permitido
		else {
			$session->tarea_actual = TAREA_SERVICIOS;
	...
	{
		if ($this->permitido) {
			return $this->crear_vista("panel/servicios", $this->cargar_datos());
	...
	private function cargar_datos()
	{
		...
		$datos['servicios'] = $tabla_servicios->datatable_servicios($session->rol_actual['clave']);
		...
		$datos['breadcrumb'] = breadcrumb_panel($navegacion, 'Servicios');
...
*/
	...
	public function estatus_servicio()
	{
		if ($this->permitido) {
			$tabla_servicios = new \App\Models\Tabla_servicios;
			if ($tabla_servicios->update($this->request->getPost('id'), array('estatus_servicio' => $this->request->getPost('estatus')))) {
	...
	public function eliminar_servicio()
	{
		if ($this->permitido) {
			$tabla_servicios = new \App\Models\Tabla_servicios;
			if ($tabla_servicios->delete($this->request->getPost('id'))) {
				mensaje("El servicio ha sido eliminado exitosamente", SUCCESS_ALERT, "¡Servicio eliminado!");
				return $this->response->setJSON(['error' => 0]);
			} //end if elimina
			else {
				mensaje("Hubo un error al eliminar el servicio, intenta nuevamente", DANGER_ALERT, "¡Error al eliminar!");
				return $this->response->setJSON(['error' => 1]);
			} //end else
		} //end if es un usuario permitido
		else {
	...
	public function recuperar_servicio()
	{
		if ($this->permitido && (session()->rol_actual['clave'] == ROL_SUPERADMIN['clave'])) {
			$mensaje = array();
			$tabla_servicios = new \App\Models\Tabla_servicios;
			if ($tabla_servicios->update($this->request->getPost('id'), array('eliminacion' => NULL))) {
				$mensaje['mensaje'] = 'El servicio se encuentra de nuevo en los registros de la base de datos.';
				$mensaje['tipo_mensaje'] = SUCCESS_ALERT;
				$mensaje['titulo'] = '¡Registro recuperado!';

				$acciones = array();
				$acciones[] = 'window.location = "./administracion_servicios";';

				mensaje("El servicio ha sido recuperado exitosamente", SUCCESS_ALERT, "¡Servicio recuperado!");
				...
				$acciones = array();
				mensaje("Hubo un error al intentar recuperar el servicio, intente de nuevo, por favor.", DANGER_ALERT, "¡Servicio no recuperado!");
